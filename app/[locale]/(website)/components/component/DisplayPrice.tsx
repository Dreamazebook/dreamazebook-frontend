...
const DisplayPrice = ({value = 0, discount, style}:DisplayPriceProps) => {
  ...
  if (typeof discount !== 'undefined') {
    return (
      <span className={`${style || 'text-gray-800'} space-x-3 whitespace-nowrap`}>
  ...
  return (
    <span className={`${style || 'text-gray-800'} whitespace-nowrap`}>{typeof value === 'number' && value < 0 ? '-' : ''}${formatedValue} USD</span>
  );
};
