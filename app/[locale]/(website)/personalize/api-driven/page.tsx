...
export default function PersonalizeApiDrivenPage() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      ...
      try {
        ...
        // Derive upload constraints from attributes.face_images.validation or customization_config
        try {
          const faceField = (product.attributes || []).find((a: any) => a?.name === 'face_images');
          const v = faceField?.validation || product?.customization_config || {};
          const allowed = Array.isArray(v?.allowed_types) && v.allowed_types.length > 0
            ? v.allowed_types
            : ['image/jpeg','image/png','image/jpg','image/webp'];
          const maxSize = typeof v?.max_file_size === 'number' ? v.max_file_size : undefined;
          const maxFiles = typeof v?.max_files === 'number' ? v.max_files : product?.customization_config?.max_face_images || 3;
  ...
  const handleContinue = async () => {
    ...
    const mapHairColorToBackend = (key: string | number): string => {
      ...
      if (v === 1) return 'blone';
      if (v === 3) return 'dark';
      // no explicit brown in backend; fallback to dark
      return 'dark';
    };
    ...
    if (rawApi && productSchema) {
      const selections: Record<string, string> = {
        // commerce field keys from backend
        // we don't render these here, but language is in query, cover/giftbox may be set elsewhere
