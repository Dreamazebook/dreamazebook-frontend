...
export default function BelovedByEarlyTesters() {
  return (
    <AnimatedSection style={{'backgroundImage':`url(${BELOVED_BG})`}} className="bg-cover">
      <Container>
        <ContainerTitle cssClass="mb-16 md:mb-32 flex items-center justify-center md:gap-3">
          <span className="text-nowrap"><span className="text-[#022CCE]">Beloved</span> By Early Testers</span>
          <Image src={BELOVED_LIKE} alt="Like" width={32} height={32} className="md:w-[64px] md:h-[64px]" />
        </ContainerTitle>
      <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 px-2.5 md:px-0 gap-22 md:gap-12">
        ...
        },
        {
          ...
          <article style={style as React.CSSProperties} className={`relative bg-cover bg-origin-padding bg-no-repeat ${cssClass}`} key={tl}>
            <Image src={img} alt={tl} width={90} height={90} className="absolute mx-auto md:w-[120px] md:h-[120px] -top-7 md:-top-14 left-0 right-0" />
            <div className="p-9">
              <h3 className="font-bold text-[#222222] text-[28px] text-center mt-10 mb-2 md:mb-4">{tl}</h3>
              <Image src={BELOVED_START} alt="Rating" width={168} height={24} className="mx-auto mb-2 md:mb-4" />
              <ContainerDesc cssClass="text-left pb-10 md:pb-35">
                {desc}
              </ContainerDesc>
            </div>
          </article>
